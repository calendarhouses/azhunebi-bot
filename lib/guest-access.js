...
async function getGuestMenuAccess(telegramUserId) {
  ...
  if (error) {
    // Table missing / schema cache — treat as no row so cabin binding fallback can work.
    if (
      error.code === "42P01" ||
      error.code === "PGRST205" ||
      /guest_menu_access/i.test(error.message || "")
    ) {
      console.error("[access] guest_menu_access read failed", error.message);
      return null;
    }
...
async function tryUpsertGuestMenuAccess(row) {
  try {
    await upsertGuestMenuAccess(row);
    return { ok: true, error: null };
  } catch (error) {
    console.error("[access] guest_menu_access upsert failed", error);
    return {
      ok: false,
...
async function restoreAccessFromActiveBinding(telegramUserId) {
  ...
  const userId = normalizeTelegramUserId(telegramUserId);
  const existing = await getGuestMenuAccess(userId).catch(() => null);
  const cabinNumber = Number(reconciled.session.cabin_number);

  await tryUpsertGuestMenuAccess({
  ...
  const access =
    (await getGuestMenuAccess(userId).catch(() => null)) ||
    ({
      telegram_user_id: userId,
      source: "cabin",
      cabin_number: cabinNumber,
...
async function userCanOrder(user) {
  ...
  let access = await getGuestMenuAccess(user.id).catch(() => null);
  if (access) {
...
async function claimGuestAccessFromStartParam(user, startParam) {
  if (await isTelegramAdmin(user)) {
    ...
    const existing = await getGuestMenuAccess(user.id).catch(() => null);
    ...
    return {
      allowed: true,
      reason: "admin",
      access: await getGuestMenuAccess(user.id).catch(() => null),
  ...
  if (!parsed) {
    ...
    return {
      ...
      access: canOrder.access || (await getGuestMenuAccess(user.id).catch(() => null)),
      location: null,
      wasNew: Boolean(canOrder.wasNew),
      persisted: Boolean(canOrder.allowed),
    };
  }
  ...
  if (parsed.type === "cabin") {
    ...
    try {
      session = await getOrCreateActiveSession(parsed.number);
      await bindUserToSession(user.id, parsed.number, session.id);
      bindOk = true;
    } catch (error) {
      console.error("[access] cabin session/bind failed", error);
    }

    const upsert = await tryUpsertGuestMenuAccess({
      telegram_user_id: userId,
      source: "cabin",
      cabin_number: parsed.number,
      table_number: null,
    ...
    if (!bindOk && !upsert.ok && !existing) {
      const error = new Error(
        upsert.error || "Не вдалося зберегти доступ по QR будиночка"
      );
    ...
    return {
      ...
      reason: "granted",
      access:
        (await getGuestMenuAccess(user.id).catch(() => null)) ||
        existing || {
          telegram_user_id: userId,
          source: "cabin",
          cabin_number: parsed.number,
      ...
      location: parsed,
      wasNew: !existing,
      persisted: bindOk || upsert.ok || Boolean(existing),
    };
  }
  ...
  if (!upsert.ok && !existing) {
    // Without guest_menu_access table, table QR cannot persist re-entry.
    return {
      allowed: true,
      reason: "granted_ephemeral",
      access: null,
      location: parsed,
      wasNew: true,
      persisted: false,
