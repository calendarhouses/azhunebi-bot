...
const DEFAULT_BOT_USERNAME = "azhunebifood_bot";
...
function getBotUsername() {
  return String(process.env.TELEGRAM_BOT_USERNAME || DEFAULT_BOT_USERNAME)
    .trim()
    .replace(/^@/, "");
}

/**
 * URL for the left Chat Menu Button.
 * Prefer Direct Link (t.me/bot/app) — Telegram applies it reliably when Main Mini App exists.
 */
function getMenuButtonWebAppUrl() {
  const configured = String(process.env.TELEGRAM_MENU_APP_LINK || "").trim();
  if (configured) {
    return configured;
  }
...
function isWebAppMenuButton(button, expectedUrl) {
  if (!button || button.type !== "web_app") {
    return false;
  }
  if (button.text && button.text !== MENU_BUTTON_TEXT) {
    return false;
  }
  const currentUrl = button.web_app?.url || "";
  return !expectedUrl || currentUrl === expectedUrl;
}
...
async function ensureDefaultWebAppMenuButton({ force = false } = {}) {
  ...
  menuButtonEnsurePromise = (async () => {
    const webAppUrl = getMenuButtonWebAppUrl();
    if (!webAppUrl) {
    ...
    if (isWebAppMenuButton(current, webAppUrl) && !force) {
      return { ok: true, updated: false, webAppUrl, current };
    }

    const setResult = await setChatMenuButton({ menu_button: menuButton });
    const refreshed = await getChatMenuButton({});
    const applied = isWebAppMenuButton(refreshed, webAppUrl);

    if (!applied) {
      console.error("[menu-button] set succeeded but button not applied", {
        setResult,
        webAppUrl,
        refreshed,
      });
      return {
        ...
        reason: "not_applied",
        hint:
          "Open BotFather → your bot → Bot Settings → Menu Button → Configure menu button → Open a Web App, set the same URL, then retry health?repair=1",
      };
    }
...
/** Per-chat override — more reliable than default for some Telegram clients. */
async function ensureChatWebAppMenuButton(chatId) {
  const webAppUrl = getMenuButtonWebAppUrl();
  if (!webAppUrl || !chatId) {
    return { ok: false, reason: "missing_web_app_url_or_chat" };
  }
  ...
  const current = await getChatMenuButton({ chat_id: chatId });
  return {
    ok: isWebAppMenuButton(current, webAppUrl),
    webAppUrl,
    current,
  };
}
...
async function sendGuestMenuWelcome(chatId, { withSticker = false } = {}) {
  ...
  try {
    ...
    console.error("[welcome] ensure default menu button failed", error);
  }

  try {
    await ensureChatWebAppMenuButton(chatId);
  } catch (error) {
    console.error("[welcome] ensure chat menu button failed", error);
  }
