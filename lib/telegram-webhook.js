...
const DEFAULT_WEBHOOK_URL = "https://azhunebi-bot-six.vercel.app/api/webhook";

let ensurePromise = null;

function resolveWebhookUrl() {
  const configured = String(process.env.WEBHOOK_URL || "").trim();
  if (configured) {
    return configured.replace(/\/$/, "");
  }

  // Prefer stable production alias over per-deployment VERCEL_URL.
  ...
  if (productionHost) {
    return `https://${productionHost}/api/webhook`;
  }

  return DEFAULT_WEBHOOK_URL.replace(/\/$/, "");
}
...
async function ensureBotWebhook(options = {}) {
  ...
  ensurePromise = (async () => {
    ...
    return {
      ...
      updated: true,
      expectedUrl,
      currentUrl: (refreshed?.url || "").replace(/\/$/, ""),
      info: refreshed,
    };
  })().catch((error) => {
    ensurePromise = null;
