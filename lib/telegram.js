...
// Sends a Lottie animation as an animated Telegram sticker (.tgs = gzipped Lottie).
// Best-effort: callers should catch errors so a failed sticker never blocks text.
async function sendLottieSticker({ chat_id, lottie }) {
  const tgs = zlib.gzipSync(Buffer.from(JSON.stringify(lottie), "utf8"));
  ...
  form.append(
    "sticker",
    new Blob([tgs], { type: "application/gzip" }),
    "sticker.tgs"
  );
