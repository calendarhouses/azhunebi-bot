...
/** start_param from signed initData (more reliable than initDataUnsafe on some clients). */
...
async function editMessageReplyMarkup(payload) {
  return telegramApi("editMessageReplyMarkup", payload);
}

async function setChatMenuButton(payload = {}) {
  const body = { ...payload };
  // Telegram expects menu_button as a JSON-serialized string (even with JSON POST).
  if (body.menu_button && typeof body.menu_button === "object") {
    body.menu_button = JSON.stringify(body.menu_button);
  }
  return telegramApi("setChatMenuButton", body);
}

async function getChatMenuButton(payload = {}) {
  return telegramApi("getChatMenuButton", payload);
}
