...
const API_VERSION = "2026-09-04-qr-access";
...
async function assertNotBlocked(user) {
  if (await isTelegramUserBlocked(user)) {
...
async function handleCreateOrder(body) {
  ...
  // Prefer start_param from the client (current QR open), then fall back to stored access.
