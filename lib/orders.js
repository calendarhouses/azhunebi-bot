...
async function createOrder({ user, cartInput, comment, locationNote, paymentMethod }) {
  const supabase = getSupabaseAdmin();
...
async function getUserOrders(telegramUserId, limit = 8) {
  ...
  const { data, error } = await supabase
    .from("orders")
    .select("id, status, total, created_at, location_note, payment_method")
    .eq("tenant_id", TENANT_ID)
...
function formatUserOrdersMessage(orders) {
  if (!orders.length) {
    return "📋 У вас поки немає замовлень.\n\nВідкрийте меню та зробіть перше замовлення!";
  }
  ...
  orders.forEach((order, index) => {
    ...
    lines.push(`${index + 1}. ${formatOrderDate(order.created_at)}`);
  ...
  lines.push("Статус оновлюється автоматично після дій адміністратора.");
