...
function formatAdminOrderMessage(order) {
  ...
  if (order.scheduled_for) {
    ...
    lines.push(`   (Готуємо автоматично за 1 год до подачі)`);
  }
...
// Short caption shown under the premium receipt photo (the rest is on the image).
function buildAdminCaption(order) {
  const username = order.user_username
    ? `@${order.user_username}`
    : "без username";
  const lines = [
    `👤 Гість: ${order.user_first_name} (${username})`,
    `Статус: ${STATUS_LABELS[order.status] || order.status}`,
  ];
...
function adminKeyboard(order) {
  if (order.status === "pending") {
    return {
      inline_keyboard: [
        [
          { text: "✅ Прийняти", callback_data: `c:${order.id}` },
  ...
  if (order.status === "preparing") {
    return {
      inline_keyboard: [[{ text: "🍽 Готово", callback_data: `r:${order.id}` }]],
    };
  }
...
async function notifyCustomerReady(order) {
  const location = order.location_note ? `\n📍 ${order.location_note}` : "";

  await sendMessage({
    chat_id: order.telegram_user_id,
    text: `🍽 Ваше замовлення готове!${location}\n\nСмачного! Дякуємо, що обрали «Аж у небі».`,
  });
}
...
async function refreshAdminMessage(order) {
  ...
  // Clear buttons on final states, otherwise keep the action keyboard.
  const replyMarkup = adminKeyboard(order) || { inline_keyboard: [] };

  // Photo notifications need editMessageCaption; text ones need editMessageText.
  // We don't track the type, so try caption first and fall back to text.
  try {
    ...
    // Likely a text message — fall through.
  }
  ...
  try {
    ...
    console.error("[order] failed to refresh admin message", textError);
  }
}
...
function formatUserOrdersMessage(orders) {
  ...
  orders.forEach((order, index) => {
    ...
    if (order.locationNote) {
      lines.push(`   📍 ${order.locationNote}`);
    }
