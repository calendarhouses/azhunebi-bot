...
const { openMenuInlineKeyboard } = require("./guest-welcome");
...
const ADMIN_ORDER_HEADERS = {
  pending: "🔔 Нове замовлення!",
  // New orders start as accepted — kitchen sees them as fresh arrivals.
  accepted: "🔔 Нове замовлення!",
  preparing: "🔥 В процесі",
  ready: "🍽 Готово",
  cancelled: "❌ Скасовано",
};

function formatAdminOrderHeader(status) {
  return ADMIN_ORDER_HEADERS[status] || ADMIN_ORDER_HEADERS.pending;
}
...
function formatAdminOrderMessage(order) {
  const username = order.user_username
    ...
    : "без username";
  const lines = [
    formatAdminOrderHeader(order.status),
...
async function createOrder({
  ...
  const { data: order, error } = await supabase
    ...
    .insert({
      tenant_id: TENANT_ID,
      // Skip kitchen "accept" step — guests see accepted immediately.
      status: "accepted",
...
async function attachOrderScreenshot(telegramUserId, orderId, screenshot) {
  ...
  await notifyAdminNewOrder(fresh);
  return fresh;
}
...
async function notifyCustomerReady(order) {
  ...
  try {
    ...
    // Never block kitchen "Готово" if the guest chat is unavailable.
    console.error("[ready] message failed", error);
  }
}
...
function isBenignTelegramEditError(error) {
...
function isPhotoCaptionEditError(error) {
  const message = String(error?.message || error || "");
  return (
...
async function refreshAdminMessage(order, messageContext) {
  ...
  // Update inline buttons first — this is what kitchen staff see.
  try {
  ...
  try {
    ...
    if (isPhotoCaptionEditError(error)) {
      try {
        ...
        if (isBenignTelegramEditError(captionError)) {
          return true;
        }
        console.error("[order] editMessageCaption failed", {
          orderId: order.id,
          messageId,
          error: captionError,
        });
...
async function handleOrderCallback(action, orderId, messageContext) {
  ...
  if (action === "ready") {
    ...
    try {
      ...
      console.error("[ready] customer notify failed", error);
    }
    return finishOrderCallback(order, messageContext, true);
  }
...
/** Admin panel: can cancel ready orders too (house bill correction). */
