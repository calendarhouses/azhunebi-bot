...
function formatLocationDisplay(cabin, table) {
  ...
  if (tableLabel && cabinLabel) {
    ...
    const deliveryLine = tableNum
      ? `Доставка — Замовлення: За столиком №${tableNum}`
      : `Доставка — ${tableLabel}`;
    return `${deliveryLine} (Проживання: ${cabinLabel})`;
  }

  if (tableLabel) {
    const match = /№\s*(\d{1,2})/.exec(tableLabel);
    if (match) {
      return `Доставка — Замовлення: За столиком №${match[1]}`;
    }
    return tableLabel;
  }

  return cabinLabel ? `Проживання: ${cabinLabel}` : null;
}
