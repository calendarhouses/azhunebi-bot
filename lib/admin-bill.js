...
async function settleAllSessionOrders(sessionId) {
  if (!sessionId) {
  ...
  for (const order of orders || []) {
    ...
    const hasOpenLines = cart.some((line) => getLineOpenQuantity(line) > 0);

    if (!hasOpenLines) {
      continue;
    }

    const settledCart = cart.map((line) => ({
      ...line,
      settledQuantity: line.quantity,
    }));

    await updateOrderCart(order.id, settledCart);
  }
