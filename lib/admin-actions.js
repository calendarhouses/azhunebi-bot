...
async function uploadDishImage(user, payload) {
  await assertAdmin(user);
  ...
  const buffer = Buffer.from(fileBase64, "base64");
  const extension = String(payload.ext || payload.fileName || "webp")
    .split(".")
    .pop()
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
  const path = `menu/dish_${Date.now()}.${extension || "webp"}`;
  const supabase = getSupabaseAdmin();
...
async function adminCancelOrder(user, payload) {
  ...
  const { cancelOrderByAdmin } = require("./orders");
  await cancelOrderByAdmin(payload.orderId);
...
async function adminUpdateOrderCartAction(user, payload) {
  await assertAdmin(user);
  return adminUpdateOrderCart(payload);
}

async function adminSettleOrderLineAction(user, payload) {
  await assertAdmin(user);
  return adminSettleOrderLine(payload);
}

async function adminSettleAllSessionOrdersAction(user, payload) {
  await assertAdmin(user);
  ...
  return settleAllSessionOrders(payload.sessionId);
}
...
async function handleAdminAction(user, action, payload) {
  switch (action) {
    ...
    case "updateOrderCart":
      return adminUpdateOrderCartAction(user, payload);
    case "settleOrderLine":
      return adminSettleOrderLineAction(user, payload);
    case "settleAllSessionOrders":
      return adminSettleAllSessionOrdersAction(user, payload);
    case "saveDish":
    ...
    case "uploadDishImage":
      return uploadDishImage(user, payload);
    case "addAdmin":
